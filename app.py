...
def load_data():
    def make_sample():
        np.random.seed(42)
        n=5000
        segs=["Champion","Loyal Customer","Potential Loyalist","At-Risk","New Customer","Hibernating","Lost"]
        props=[0.15,0.20,0.18,0.20,0.12,0.10,0.05]
        cats=["Dairy & Eggs","Fresh Produce","Meat & Seafood","Pantry & Dry Goods","Beverages","Household","Snacks & Cereal"]
        return pd.DataFrame({
...
tab1,tab2,tab3,tab4,tab5 = st.tabs([
    "📊 Overview","👥 Segments","🔍 Customers","🎯 Recommendations","🍳 Recipes"
])
...
with tab1:
    ...
    for col, val, color, label in kpi_data:
        with col:
            st.markdown(f'<div class="kpi-card"><p class="kpi-value" style="color:{color}">{val}</p><p class="kpi-label">{label}</p></div>', unsafe_allow_html=True)
    ...
    with col_a:
        ...
        fig1 = px.bar(seg_df,x="Count",y="Segment",orientation="h",
                      color="Segment",color_discrete_map=SEG_COLORS,text="Count",height=340)
    ...
    with col_c:
        ...
        fig3 = px.bar(sp_df,x="Avg Spend",y="Segment",orientation="h",
                      color="Segment",color_discrete_map=SEG_COLORS,text="Avg Spend",height=340)
    ...
    if "r_score" in filtered.columns and "f_score" in filtered.columns:
        ...
        fig5  = px.imshow(pivot,color_continuous_scale="YlOrRd",text_auto=True,height=320,
                          labels=dict(x="Frequency Score (1→5)",y="Recency Score (1→5)",color="Avg Spend ($)"))
...
with tab2:
    ...
    seg_tbl["Pct_of_Base"] = (seg_tbl["Customers"]/seg_tbl["Customers"].sum()*100).round(1)
    ...
    STRAT = {
        ...
        "New Customer":       ("🌱","#5DCAA5","Onboard & nurture","Welcome email with first-order discount. Popular product bundles."),
        "Hibernating":        ("😴","#888780","Wake them up","Last chance re-engagement with 20% discount on top category."),
    ...
    for i,(seg,(icon,color,action,desc)) in enumerate(STRAT.items()):
        ...
        with cols_s[i%2]:
            st.markdown(f'<div style="border:1px solid {color}33;border-left:4px solid {color};border-radius:8px;padding:10px 14px;margin-bottom:10px;background:{color}0a"><span style="font-size:14px">{icon}</span><strong style="color:{color};margin-left:6px">{seg}</strong><span style="float:right;font-size:11px;color:#666">{cnt:,} customers</span><div style="font-size:12px;font-weight:600;color:#333;margin-top:5px">{action}</div><div style="font-size:11px;color:#555;line-height:1.5;margin-top:3px">{desc}</div></div>', unsafe_allow_html=True)
...
with tab3:
    ...
    disp = [c for c in ["customer_id","segment","recency","frequency","monetary","r_score","f_score","m_score","top_category","avg_order_value"] if c in filtered.columns]
    tdf  = filtered[disp].copy()
    tdf.columns = [c.replace("_"," ").title() for c in disp]
    if search_id: tdf = tdf[tdf["Customer Id"].str.contains(search_id.upper(),na=False)]
    if seg_filter != "All": tdf = tdf[tdf["Segment"]==seg_filter]
    sc = sort_by.replace("_"," ").title()
    if sc in tdf.columns: tdf = tdf.sort_values(sc,ascending=False)
...
with tab4:
    ...
    with col_r1:
        if not recs.empty:
            cid = st.selectbox("Select a customer",options=recs["customer_id"].unique()[:100])
                ...
                for i in range(1,6):
                    ...
                    if cn in cr.columns:
                        ...
                        if v and str(v).strip():
                            st.markdown(f'<div style="background:#F0F7FF;border-left:3px solid #0071CE;padding:8px 12px;border-radius:0 6px 6px 0;margin-bottom:6px;font-size:13px">🛒 {v}</div>',unsafe_allow_html=True)
        else:
    ...
    with col_r2:
        if not rules.empty:
            ...
            for _,row in rules.head(10).iterrows():
                st.markdown(f'<div style="background:#F8F9FA;border:1px solid #e0e0e0;padding:8px 12px;border-radius:6px;margin-bottom:6px;font-size:12px">🛍 <strong>{str(row["product_1"])[:35]}</strong> <span style="color:#0071CE">→</span> 🛍 <strong>{str(row["product_2"])[:35]}</strong> <span style="float:right;color:#1D9E75;font-weight:600">{float(row["confidence"]):.0%}</span></div>',unsafe_allow_html=True)
        else:
...
with tab5:
    ...
    if not recipes.empty:
        ...
        with rc1:
            rcid = st.selectbox("Select customer",options=recipes["customer_id"].unique()[:100])
            minp = st.slider("Minimum match %",0,100,20)
        with rc2:
            cr2 = recipes[(recipes["customer_id"]==rcid)&(recipes["match_pct"]>=minp)].sort_values("match_pct",ascending=False)
            if not cr2.empty:
                for _,row in cr2.iterrows():
                    pct=float(row["match_pct"])
                    col=("#1D9E75" if pct>=70 else "#EF9F27" if pct>=40 else "#185FA5")
                    st.markdown(f'<div style="border:1px solid #e0e0e0;border-radius:10px;padding:12px 16px;margin-bottom:10px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><span style="font-size:20px">🍳</span><strong style="font-size:14px">{row["recipe_name"]}</strong><span style="margin-left:auto;background:{col};color:white;padding:2px 10px;border-radius:12px;font-size:11px;font-weight:600">{pct:.0f}% match</span><span style="font-size:11px;color:#666">⏱ {row["minutes"]} min</span></div><div style="font-size:11px"><span style="color:#1D9E75">✓ Have: {str(row["have_items"])[:100]}</span><br><span style="color:#E24B4A">+ Need: {str(row["need_items"])[:100]}</span></div></div>',unsafe_allow_html=True)
            else:
                st.info("No recipes found above the minimum match threshold.")
    else:
        st.info("Upload recipe_matches.csv to see recipe suggestions.")

st.markdown("---")
st.markdown('<div style="text-align:center;color:#888;font-size:11px;padding:8px 0">SmartCart — Walmart Grocery RFM Analytics · Project 2024 · Streamlit + Plotly</div>',unsafe_allow_html=True)
